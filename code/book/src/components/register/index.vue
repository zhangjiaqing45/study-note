<template>
	<div class="register_body">
		...
		<div>
			用户名：
			<input v-model="username" class="register_text" type="text" placeholder="不可重复,不可修改"/>
		</div>
		<div>
			密码：
			<input v-model="password" name='password' class="register_text" type="password"/>
		</div>
		<div>
			学院：
		...
		<div>
			联系方式：
			<input v-model="tel" class="register_text" type="text" placeholder="请在前面注明手机/qq/wechat,如qq123456789" />
		</div>
		<div>
			验证码：
			<input v-model="verify" class="register_text" type="text" />
		</div>
		<div class="register_btn"><button @touchstart="handleToRegister">注册</button></div>
...
export default {
	...
	data() {
		return {
			selectedOption: '',
			options: ['管理学院', '经济学院', '计算机与通信工程学院', '数学与统计学院', '外国语言文化学院', '资源与材料学院', '控制工程学院', '研究生分院'],
			email: '',
			username: '',
			password: '',
			academy: '',
